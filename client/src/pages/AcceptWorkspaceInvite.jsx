...
export default function AcceptWorkspaceInvite() {
  ...
  useEffect(() => {
    if (user && inviteInfo && !accepted && !accepting) accept();
  }, [user, inviteInfo]);
  ...
  async function accept() {
    ...
    try {
      ...
      setBoardCount(res.boardCount || 0);
      setAccepted(true);
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-indigo-800 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-slide-up">
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-violet-600 px-8 py-7 text-center">
          ...
          <p className="text-indigo-200 text-sm mt-1">Full access to all boards</p>
        </div>
        ...
        <div className="px-8 py-6">
          {loading ? (
            ...
            <div className="text-center space-y-3 py-4">
              ...
              <div>
                <p className="font-bold text-slate-800 dark:text-slate-200 text-lg">You're in!</p>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                  Added to <strong>{boardCount}</strong> board{boardCount !== 1 ? 's' : ''}. Taking you to your workspace…
                </p>
              </div>
            </div>
            ...
            <div className="space-y-5">
              <div className="bg-slate-50 dark:bg-slate-700/40 rounded-xl p-5 border border-slate-100 dark:border-slate-700">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Workspace access</p>
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full capitalize ${ROLE_COLOR[inviteInfo.role] || ROLE_COLOR.member}`}>
                    {inviteInfo.role}
                  </span>
                </div>
                <div className="space-y-2">
                  {['Access to all existing boards', 'Auto-added to new boards', 'Create and manage your own boards'].map(perk => (
              ...
              {!user ? (
                <div className="space-y-3">
                  ...
                  <Link
                    to={`/register?redirect=/workspace-invite/${token}`}
                  ...
                  <Link
                    to={`/login?redirect=/workspace-invite/${token}`}
